...
def simpleTokenize(string):
    ...
    """
    tokens = re.compile(split_regex).split(string)
...
# print amazonRecToToken.mapValues(len).takeOrdered(1, key=lambda (id, n_tokens): -n_tokens)
...
def tf(tokens):
    ...
    """
#     print tokens
    tokens_dct = {token:tokens.count(token) * 1.0 / len(tokens) for token in tokens}
#     print tokens_dct
    return tokens_dct
...
def norm(a):
    ...
    Returns:
        norm: norm of a dictionary of tokens to its TF values
    """
...
# print amazonFullRecToToken.take(5)
...
amazonInvPairsRDD = (amazonWeightsRDD
                    .flatMap(invert)
                    .cache())
# print amazonInvPairsRDD.take(5)
...
# print amazonInvPairsRDD.filter(lambda(token, id): token in ['aided', 'clickart']).collect()
# print googleInvPairsRDD.filter(lambda(token, id): token in ['aided', 'clickart']).collect()
commonTokens = (amazonInvPairsRDD
                .join(googleInvPairsRDD)
                .map(swap)
                .groupByKey()
                .cache())
# print commonTokens.filter(lambda(id_tpl, token): token in ['aided', 'clickart']).collect()
...
#     print amazonWeightsBroadcast.value[amazonRec]
#     print dir(amazonWeightsBroadcast)
...
# sTest = commonTokens.filter(lambda ((aID, gURL), tokens): aID == 'b00005lzly' and gURL == 'http://www.google.com/base/feeds/snippets/13823221823254120257').collect()
# print sTest[0]
# sFCS = fastCosineSimilarity(sTest[0]) 
# print sFCS
# Test.assertTrue(abs(sFCS[1] - 4.286548414e-06) < 0.000000000001, 'incorrect similarityTest fastCosineSimilarity')
