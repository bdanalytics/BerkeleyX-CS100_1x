...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    RatingsN = len(IDandRatingsTuple[1])
    RatingsAvg = sum(IDandRatingsTuple[1]) * 1.0 / RatingsN
    retval = (MovieID, (RatingsN, RatingsAvg))
#     print retval
    return retval
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              ...
                              .map(lambda(MovieID, MovieInfoTpl): 
                                   (MovieInfoTpl[1][1], MovieInfoTpl[0], MovieInfoTpl[1][0]))
                             )
...
movieLimitedAndSortedByRatingRDD = (movieNameWithAvgRatingsRDD
                                    .filter(lambda(RatingsAvg, MovieName, RatingsN): RatingsN > 500)
...
#     print predictedReformattedRDD.take(3)
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda(keyTpl, RatingDiffTpl): 
                                    (keyTpl, (RatingDiffTpl[1] - RatingDiffTpl[0]) ** 2.0)))
#                                     (keyTpl, (RatingDiffTpl[1] - RatingDiffTpl[0]) * 1.0)))    
#     print squaredErrorsRDD.collect()

    # Compute the total squared error - do not use collect()
    from operator import add
    totalError = (squaredErrorsRDD
                      .map(lambda(keyTpl, squaredError): squaredError)
                      .reduce(add))
#     print totalError
...
#     print numRatings
#     print (totalError / float(numRatings))
    
    retVal = (totalError / float(numRatings)) ** 0.5    
#     print retVal
...
testForAvgRDD = testRDD.map(lambda(UserID, MovieID, Rating): 
                            (UserID, MovieID, trainingAvgRating))
...
# myMoviesRatingsRDD = (movieLimitedAndSortedByRatingRDD
#                       .filter(lambda(Rating, MovieName, NumRatings): 
#                                Rating >= 4.3)                      
#                       .map(lambda(Rating, MovieName, NumRatings): 
#                            (MovieName, Rating, NumRatings))
#                       )
# print myMoviesRatingsRDD.take(20)
# # print moviesRDD.take(5)
# myMoviesNamesRDD = moviesRDD.map(lambda(MovieID, MovieName): (MovieName, MovieID))
# print myMoviesNamesRDD.take(5)
# myMoviesRDD = (myMoviesRatingsRDD
#                 .leftOuterJoin(myMoviesNamesRDD))
# print myMoviesRDD.take(20)
...
myUnratedMoviesRDD = (moviesRDD
                      .map(lambda(MovieID, MovieName): (0, MovieID)))
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda(MovieID, MovieInfoTpl): 
                                   (MovieID, MovieInfoTpl[0]))

# Transform predictedRatingsRDD into an RDD with entries that are pairs of the form (Movie ID, Predicted Rating)
predictedRDD = predictedRatingsRDD.map(lambda(UserID, MovieID, Rating): 
                                               (MovieID, Rating))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .filter(lambda(MovieID, MovieInfoTpl): MovieInfoTpl[1] > 75)
                       .leftOuterJoin(moviesRDD)
                       .map(lambda(MovieID, MovieInfoTpl): 
                            (MovieInfoTpl[0][0], MovieInfoTpl[1], MovieInfoTpl[0][1]))
                       )
# print ratingsWithNamesRDD.take(5)
