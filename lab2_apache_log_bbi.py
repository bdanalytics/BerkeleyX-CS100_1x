...
print APACHE_ACCESS_LOG_PATTERN
...
#dir(os.path)
# print os.path.sys()
# MY_ACCESS_LOG_PATTERN = '^(\S+) (\S+) (\S+) \[([\w:/]+\s[+\-]\d{4})\] "(\S+) (\S+)\s*(\S*)" (\d{3}) (\S+)'
# print MY_ACCESS_LOG_PATTERN
# vld_line = '127.0.0.1 - - [01/Aug/1995:00:00:01 -0400] "GET /images/launch-logo.gif HTTP/1.0" 200 1839'
# print vld_line
# vld_match = re.search(MY_ACCESS_LOG_PATTERN, vld_line)
# for i in xrange(1, 10):
#     print 'vld_match[%d]=%s' % (i, vld_match.group(i))


# In[5]:

TST_ACCESS_LOG_PATTERN = '^(\S+) (\S+) (\S+) \[([\w:/]+\s[+\-]\d{4})\] "(\S+) (\S+)\s*(\S*)\s*" (\d{3}) (\S+)'
print TST_ACCESS_LOG_PATTERN
ivld_line = 'ix-sac6-20.ix.netcom.com - - [08/Aug/1995:14:47:48 -0400] "GET /shuttle/countdown/count.html HTTP/1.0 " 200 73231'
print ivld_line
ivld_match = re.search(TST_ACCESS_LOG_PATTERN, ivld_line)
for i in xrange(1, 10):
    print 'ivld_match[%d]=%s' % (i, ivld_match.group(i))
...
not200 = (access_logs
             .map(lambda log: (log.response_code, log.endpoint, 1))
             .filter(lambda rc_tpl: rc_tpl[0] != 200)
             .cache())
#print not200.take(5)

endpointCountPairTuple = (not200
                          .map(lambda rc_tpl: (rc_tpl[1], rc_tpl[2])))
#print endpointCountPairTuple.take(5)

endpointSum = endpointCountPairTuple.reduceByKey(lambda a, b: a + b)

topTenErrURLs = endpointSum.takeOrdered(10, lambda rc_tpl: -1 * rc_tpl[1])
...
print dayToHostPairTuple.take(5)

dayGroupedHosts = dayToHostPairTuple.groupByKey()
print dayGroupedHosts.take(5)
...
# chkList = [(1, 2582), (3, 3222), (4, 4190), (5, 2502), (6, 2537), (7, 4106), (8, 4406), (9, 4317), (10, 4523), (11, 4346), (12, 2864), (13, 2650), (14, 4454), (15, 4214), (16, 4340), (17, 4385), (18, 4168), (19, 2550), (20, 2560), (21, 4134), (22, 4456)]
# Test.assertEquals(dailyHostsList[1], chkList[1], 'incorrect dailyHostsList')
# Test.assertEquals(dailyHostsList[1:21], chkList[1:21], 'incorrect dailyHostsList')
# Test.assertEquals(dailyHostsList, chkList, 'incorrect dailyHostsList')
# print len(chkList)
...
daysWithHosts = dailyHosts.map(lambda (tpl): tpl[0]).collect()
hosts = dailyHosts.map(lambda (tpl): tpl[1]).collect()
...
sortedByDay = groupedByDay.sortBy(lambda tpl: tpl[0])
print sortedByDay.take(5)

avgDailyReqPerHost = (sortedByDay
                        .mapValues(len)
                        .join(dailyHosts)
                        .map(lambda (tpl): (tpl[0], tpl[1][0] / tpl[1][1]))
                        .sortBy(lambda (tpl): tpl[0])
                        .cache()
                     )
...
daysWithAvg = avgDailyReqPerHost.map(lambda (tpl): tpl[0]).collect()
avgs = avgDailyReqPerHost.map(lambda (tpl): tpl[1]).collect()
...
badEndpoints = (badRecords
                .map(lambda (log): (log.endpoint, 1))
                .reduceByKey(add)
                .sortBy(lambda (tpl): -tpl[1])
                .cache()
               )
#print badEndpoints.take(5)

badUniqueEndpoints = badEndpoints.map(lambda (tpl): tpl[0])
...
badEndpointsTop20 = badEndpoints.take(20)
...
errHostsTop25 = errHosts.take(25)
...
errDateSorted = (badRecords
                 .map(lambda (log): (log.date_time.date().day, 1))
                 .reduceByKey(add)
                 .sortBy(lambda (tpl): tpl[0])
                 .cache())
...
daysWithErrors404 = errDateSorted.map(lambda (tpl): tpl[0]).collect()
errors404ByDay = errDateSorted.map(lambda (tpl): tpl[1]).collect()
...
topErrDate = errDateSorted.sortBy(lambda (tpl): -tpl[1]).take(5)
...
hourRecordsSorted = (badRecords
                     .map(lambda (log): (log.date_time.time().hour, 1))
                     .reduceByKey(add)
                     .sortBy(lambda (tpl): tpl[0])
                     .cache())
...
hoursWithErrors404 = hourRecordsSorted.map(lambda (tpl): tpl[0]).collect()
errors404ByHours = hourRecordsSorted.map(lambda (tpl): tpl[1]).collect()
